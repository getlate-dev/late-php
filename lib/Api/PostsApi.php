...
class PostsApi
{
    ...
    {
        list($response) = $this->editPostWithHttpInfo($post_id, $edit_post_request, $contentType);
        return $response;
    }

    /**
     * Operation editPostWithHttpInfo
     *
     * Edit published post
     *
     * @param  string $post_id (required)
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function editPostAsync($post_id, $edit_post_request, string $contentType = self::contentTypes['editPost'][0])
    {
        return $this->editPostAsyncWithHttpInfo($post_id, $edit_post_request, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    /**
     * Operation editPostAsyncWithHttpInfo
     *
     * Edit published post
     *
     * @param  string $post_id (required)
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function editPostRequest($post_id, $edit_post_request, string $contentType = self::contentTypes['editPost'][0])
    {

        // verify the required parameter 'post_id' is set
        if ($post_id === null || (is_array($post_id) && count($post_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $post_id when calling editPost'
            );
        }

        // verify the required parameter 'edit_post_request' is set
        if ($edit_post_request === null || (is_array($edit_post_request) && count($edit_post_request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $edit_post_request when calling editPost'
            );
        }


        $resourcePath = '/v1/posts/{postId}/edit';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($edit_post_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($edit_post_request));
            } else {
                $httpBody = $edit_post_request;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
     ...
     * @param  string|null $profile_id Filter posts to a specific profile (24-char hex ObjectId). Omit it, or send &#x60;all&#x60; or an empty value, to list posts across every profile. (optional)
     * @param  string|null $created_by Filter posts to those created by a specific team user (24-char hex ObjectId). (optional)
     * @param  \DateTime|null $date_from Zero-padded YYYY-MM-DD, or a full ISO 8601 datetime. An empty value means no date filter; any other malformed value returns 400. (optional)
     * @param  \DateTime|null $date_to Zero-padded YYYY-MM-DD, or a full ISO 8601 datetime. An empty value means no date filter; any other malformed value returns 400. (optional)
