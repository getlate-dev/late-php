...
class SendInboxMessageRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets attachment_url
     *
     * @param string|null $attachment_url URL of the attachment to send (image, video, audio, or file). The URL must be publicly accessible. For binary file uploads, use multipart/form-data instead. On WhatsApp, combining an image, video, or file with `buttons` renders the media as the header of one interactive reply-button message; audio cannot be combined with buttons.
     *
     * @return self
     */
     ...
     * Sets reply_to
     *
     * @param string|null $reply_to Platform message ID to quote-reply to. For WhatsApp, pass the wamid; for Telegram, the Telegram message ID (delivered as message.platformMessageId on webhooks, and as `id` on each entry of the list-messages endpoint). On Slack it threads the reply (thread_ts) instead of quoting. Silently ignored on platforms without send-side reply support, including Instagram and Facebook Messenger (Meta's Send API rejects reply_to on Instagram and does not expose it on Messenger).
     *
     * @return self
     */
